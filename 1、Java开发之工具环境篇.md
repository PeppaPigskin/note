...
```markdown
# 说明
Omi录屏专家 for mac让你快速记录自己的工作步骤。无论是整个屏幕,还是划定的区域,或者来自摄像头的画面,都能一键录制。你的视频文件将被保存为mp4格式。
...
-- CheckStyle-IDEA————一次性检测出无用导入、没写注释、语法错误、方法太长等这些问题
    1、安装完插件后，在idea的下方会出现：CheckStyle选项：
    2、点击左边的绿色按钮，可以扫描代码。在中间位置，会显示不符合代码规范的原因。
    3、双击代码，即可直接跳转到具体代码：
